        ...
        with tab1:
            ...
            if selected_kpis:
                ...
                if interactive_chart:
                    ...
                    # Replace the existing Slack upload button with the new export functionality
                ...
                if interactive_ev_chart:
                    ...
                    with col1:
                        slack_message = st.text_input(
                            ...
                            key="ev_slack_message"
                        )
                    with col2:
                        if st.button("Export Current Chart to Slack", key="export_ev"):
                            # Check which chart type is currently displayed
                            current_chart_type = chart_type  # This is from the radio selection
                            
                            if current_chart_type == "Static (Matplotlib)":
                                # Use the existing Matplotlib export functionality
                                static_chart = create_static_ev_added_chart(
                                    df,
                                    start_date=start_date,
                                    end_date=end_date,
                                    date_format=date_format.replace("'", ""),
